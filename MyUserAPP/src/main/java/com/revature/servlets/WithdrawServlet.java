...
public class WithdrawServlet extends HttpServlet {
	...
	protected void doPost(HttpServletRequest req, HttpServletResponse res) 
			...
			try {
				...
				int result = statement.executeUpdate("UPDATE user_cmp SET balance= balance-'"+amountToBeWithdrawn+"' WHERE email= '"+email+"'");                   
				// an insert statement affects 1 row, but an update or delete statement can affect more...	
				PrintWriter out = res.getWriter();
				if (result > 0) {
					out.println("<h1>Congrats, you have withdrawn " + amountToBeWithdrawn + "!</h1>");
				} else {
					out.println("<h1>Error withdrawing...</h1>");
				}
			} catch (SQLException e) {
